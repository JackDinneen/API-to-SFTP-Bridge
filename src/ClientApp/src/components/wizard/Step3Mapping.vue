...
const AUTO_DETECT_PATTERNS: Record<string, RegExp[]> = {
  ...
  'Asset name': [/building_name/i, /asset_name/i, /site_name/i, /property_name/i],
  'Submeter Code': [/meter_id/i, /submeter/i, /meter_code/i, /meter_number/i],
  'Utility Type': [/utility_type/i, /utility/i, /energy_type/i, /resource_type/i],
  Year: [/year/i],
  Month: [/month/i],
...
const isValid = computed(() =>
  mappings.value.every((m) => m.sourcePath.trim() || m.transformType === 'Static Value'),
)

watch(
  mappings,
  (val) => {
    wizard.setStepData('mappings', val.map((m) => ({ ...m })))
...
<template>
  <div class="space-y-6">
    ...
    <p class="text-sm text-gray-600">
      ...
      <span class="bg-yellow-400 text-yellow-900 text-xs font-bold px-1 py-0.5 rounded-full">*</span>
      were auto-detected.
    </p>

    <div class="bg-white border border-gray-200 rounded-lg p-4">
      <div class="grid grid-cols-12 gap-3 pb-2 border-b border-gray-300 mb-1">
        <div class="col-span-3 text-xs font-semibold text-gray-500 uppercase">Target Column</div>
        <div class="col-span-4 text-xs font-semibold text-gray-500 uppercase">Source Field</div>
        <div class="col-span-2 text-xs font-semibold text-gray-500 uppercase">Transform</div>
        <div class="col-span-3 text-xs font-semibold text-gray-500 uppercase">Config</div>
      </div>
