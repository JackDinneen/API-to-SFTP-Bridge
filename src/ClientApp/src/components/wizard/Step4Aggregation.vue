...
const aggregationMethods = [
  {
    value: 'sum',
    label: 'Sum',
    description: 'Add all values within each month',
  },
  {
    value: 'average',
    label: 'Average',
    description: 'Average of all values within each month',
  },
  {
    value: 'last',
    label: 'Last',
    description: 'Use the last recorded value in each month',
  },
  {
    value: 'max',
    label: 'Max',
    description: 'Use the maximum value in each month',
  },
] as const
...
<template>
  <div class="space-y-6">
    ...
    <p class="text-sm text-gray-600">
      Obi requires monthly data. If your source provides sub-monthly readings
      (daily, hourly, etc.), configure how values should be aggregated into
      monthly totals.
    </p>
      ...
      <div class="border border-green-200 rounded-lg p-4 bg-green-50">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-medium text-green-800">
            No aggregation needed -- source data is already at monthly
            granularity.
          </span>
        </div>
      </div>
